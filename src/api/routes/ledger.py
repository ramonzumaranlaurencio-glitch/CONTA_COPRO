...
def _document_to_old_payload(document: FinancialDocument) -> dict:
    ...
    return {
        ...
        "partner_nit": metadata.get("customer_nit") or metadata.get("supplier_nit") or metadata.get("customer_ruc") or metadata.get("supplier_ruc"),
        "customer_nit": metadata.get("customer_nit") or metadata.get("customer_ruc"),
        "supplier_nit": metadata.get("supplier_nit") or metadata.get("supplier_ruc"),
        "metadata": metadata,
    }
        ...
        for entry in entries:
            ...
            response.append(
                JournalEntryListItem(
                    ...
                    row_hash=entry.row_hash,
                    previous_hash=entry.previous_hash,
                    dian_status=getattr(entry, "dian_status", getattr(entry, "status", "POSTED")),
    ...
    async with build_uow_factory()(ctx["tenant_id"]) as uow:
        ...
        if document is None:
            document = FinancialDocument(
                ...
                retefuente_amount=Decimal("0.00"),
                reteiva_amount=Decimal("0.00"),
