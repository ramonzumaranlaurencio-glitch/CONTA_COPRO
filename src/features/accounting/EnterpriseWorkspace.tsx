...
import { PurchaseFormEnterprise } from './PurchaseFormEnterprise';
...
const toCsv = (data: Array<Record<string, unknown>>) => {
  ...
  for (const item of data) {
    lines.push(headers.map((header) => `"${String(item[header] ?? '').replaceAll('"', '""')}"`).join(','));
  }
  return lines.join('\n');
};
...
export const EnterpriseWorkspace = () => {
  ...
  const refreshJournal = async () => {
    if (!token) {
      setStatusMessage('No hay token para refrescar datos.');
      return;
    }
    try {
      await loadJournal(token);
      setStatusMessage('Libro diario actualizado.');
    } catch {
      setStatusMessage('No se pudo refrescar libro diario.');
    }
  };
  ...
  const postPurchase = async () => {
    ...
    const subtotal = toNumber(purchaseForm.subtotal);
    const igv = toNumber(purchaseForm.igv);
    ...
    try {
      const payload = {
        ...
        year: 2026,
        month: 5,
        purchase_id: `${purchaseForm.serie}-${purchaseForm.number}`,
        supplier_ruc: purchaseForm.supplierRuc,
        doc_type: '01',
        serie: purchaseForm.serie,
        number: purchaseForm.number,
        subtotal,
        igv,
        total,
        currency: 'PEN',
        expense_account: purchaseForm.expenseAccount,
        cost_center: purchaseForm.costCenter,
      };
      ...
      setStatusMessage(`Compra ${purchaseForm.serie}-${purchaseForm.number} posteada.`);
      setActivePanel(null);
      await loadJournal(token);
    } catch {
      setStatusMessage('No se pudo postear compra.');
    }
  };
